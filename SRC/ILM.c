...
/**********/
void ILM_load_file(ILM *IL, char *file)//need to check file exist and file_size < MEMORY
{
    uint8_t *buf = NULL;
...
void ILM_execute_ins(ILM *IL)
{
    switch(IL->MEM[IL->PC])
    {
        ...
        default:
            TeddyAddLine("[!] UNKNOW INSTRUCTION FOUND, PROGRAM HALTED !!!");
...
void ILM_NOP(ILM *IL)
{
    ...
    TeddyAddLine("[!] NOP INSTRUCTION FOUND, PROGRAM HALTED !!!");
    char *str=TeddyReadLine();
    if(str)(free(str));    
    IL->VMODE=2;
}
...
void ILM_PRS(ILM *IL)
{
    ...
    while(IL->LINES[IL->CURSOR_LINE][IL->CURSOR]!='"')
    {
        ...
        TeddyAddChar((char)IL->LINES[IL->CURSOR_LINE][IL->CURSOR]);
